...
function renderInterruptStateItem(value: any): React.ReactNode {
  if (isComplexValue(value)) {
    return <code className="bg-muted/50 rounded px-2 py-1 font-mono text-sm">{JSON.stringify(value, null, 2)}</code>;
  } else if (isUrl(value)) {
    return (
      <a
        href={value}
...
export function GenericInterruptView({ interrupt }: { interrupt: Record<string, any> | Record<string, any>[] }) {
                  ...
                  {displayEntries.map((item, argIdx) => {
                    ...
                    return (
                      <tr key={argIdx}>
                        ...
                        <td className="text-muted-foreground px-4 py-2 text-sm">{renderInterruptStateItem(value)}</td>
                      </tr>
                    );
                  })}
                </tbody>
