...
export function HumanMessage({ isLoading, message }: { message: Message; isLoading: boolean }) {
  ...
  return (
    <div className={cn('group ml-auto flex max-w-full min-w-0 items-center gap-2', isEditing && 'w-full max-w-xl')}>
      <div className={cn('flex min-w-0 flex-col gap-2', isEditing && 'w-full')}>
        {isEditing ? (
          ...
          <div className="flex flex-col gap-2">
            ...
            {contentString ? (
              <p className="bg-muted ml-auto w-fit max-w-full rounded-3xl px-4 py-2 text-right [overflow-wrap:anywhere] whitespace-pre-wrap">
                {contentString}
              </p>
            ) : null}
          </div>
