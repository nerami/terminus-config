...
describe('buildAreasGraph', () => {
  it('creates one node per area with counts', () => {
    const { edges, nodes } = buildAreasGraph(topology);
  ...
  it('adds an Unassigned area when items have no area', () => {
    const t: Topology = {
      ...topology,
      entities: [{ ...topology.entities[0], area_id: null }],
    };
...
describe('buildAreaGraph', () => {
  it('links scenes and automations to the entities they use', () => {
    const { edges, nodes } = buildAreaGraph(topology, 'living');
...
describe('buildAutomationGraph', () => {
  it('parses trigger/condition/action structure into a flow graph', () => {
    ...
    const { edges, nodes } = buildAutomationGraph(topology, automation, detail);
    ...
    expect(edges.some((e) => e.source === 'action/0' && e.target === 'light.lamp')).toBe(true);
    // trigger feeds the automation root
    expect(edges.some((e) => e.source === 'trigger/0' && e.target === 'automation.night')).toBe(true);
    // the gating flow: root -> condition -> action
    expect(edges.some((e) => e.source === 'automation.night' && e.target === 'condition/0')).toBe(true);
    expect(edges.some((e) => e.source === 'condition/0' && e.target === 'action/0')).toBe(true);
  });
