...
describe('triggers + top-level conditions', () => {
  it('feeds each trigger into the root and chains conditions, then actions', () => {
    ...
    expect(model.nodes.find((n) => n.id === 'condition/0')?.entities).toContain('input_boolean.home');
  });
        ...
        condition: [
          {
            ...
            conditions: [
              ...
              {
                condition: 'or',
                conditions: [{ condition: 'state', entity_id: 'c.d', state: 'on' }, '{{ true }}'],
              },
            ],
          },
        ],
...
describe('choose', () => {
  it("paths options' conditions/sequence and the default sibling correctly", () => {
        ...
        action: [
          {
            choose: [
              {
                conditions: [{ condition: 'state', entity_id: 'a.b', state: 'on' }],
                sequence: [{ service: 'light.turn_on', target: { entity_id: 'light.lamp' } }, { delay: '00:00:05' }],
              },
            ],
    ...
    expect(hasEdge(model, 'action/0/choose/0/conditions/0', 'action/0/choose/0/sequence/0')).toBe(true);
...
describe('repeat', () => {
  it('paths the body sequence and while/until conditions, with a loop-back', () => {
    ...
    const back = model.edges.find((e) => e.source === 'action/0/repeat/sequence/0' && e.target === 'action/0');
  ...
  it('renders the body once for a count loop', () => {
    const model = buildAutomationFlow(
      {
        action: [{ repeat: { count: 3, sequence: [{ service: 'light.toggle' }] } }],
      },
      ROOT,
    );
    const bodySteps = [...ids(model)].filter((id) => id.startsWith('action/0/repeat/sequence/'));
...
describe('parallel', () => {
  ...
  it('handles a branch given as a raw list of steps', () => {
    const model = buildAutomationFlow(
      {
        action: [{ parallel: [[{ service: 'x.a' }, { service: 'x.b' }], { service: 'x.c' }] }],
      },
      ROOT,
    );
    expect(ids(model).has('action/0/parallel/0/sequence/0')).toBe(true);
...
describe('sequence + stop + inline condition', () => {
  it('paths a nested sequence block', () => {
    const model = buildAutomationFlow({ action: [{ sequence: [{ service: 'x.y' }, { service: 'x.z' }] }] }, ROOT);
  ...
  it('treats an inline condition as a gate and stop as a terminator', () => {
    const model = buildAutomationFlow(
      {
        action: [{ condition: 'state', entity_id: 'a.b', state: 'on' }, { stop: 'done' }, { service: 'never.reached' }],
      },
      ROOT,
    );
...
describe('action type precedence + leaf references', () => {
  it('classifies scene and service references as relationship leaves', () => {
    const model = buildAutomationFlow(
      {
        action: [{ scene: 'scene.movie' }, { service: 'scene.turn_on', target: { entity_id: 'scene.away' } }],
      },
      ROOT,
    );
  ...
  it('honors ACTIONS_MAP precedence (delay before service)', () => {
    const model = buildAutomationFlow({ action: [{ delay: '00:00:01', service: 'light.turn_on' }] }, ROOT);
