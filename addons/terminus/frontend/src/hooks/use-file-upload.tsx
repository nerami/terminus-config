...
export const SUPPORTED_FILE_TYPES = ['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'application/pdf'];
...
export function useFileUpload({ initialBlocks = [] }: UseFileUploadOptions = {}) {
  ...
  const isDuplicate = (file: File, blocks: ContentBlock.Multimodal.Data[]) => {
    ...
    if (SUPPORTED_FILE_TYPES.includes(file.type)) {
      return blocks.some((b) => b.type === 'image' && b.metadata?.name === file.name && b.mimeType === file.type);
    }
    return false;
  };
  ...
  const handleFileUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const newBlocks = uniqueFiles.length ? await Promise.all(uniqueFiles.map(fileToContentBlock)) : [];
