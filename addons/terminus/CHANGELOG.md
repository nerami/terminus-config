...
- **Scoped error recovery for the chat and topology panes.** A render crash in
  one region — a topology renderer, the message list, or the artifact view —
  now shows a small recoverable error in just that pane (with a Reload button)
  instead of taking down the whole screen. The 2D and 3D topology views also
  load and fail independently.

## 0.17.0

- **Resizable chat / topology split.** When both panels are open you can drag
  the divider between the chat and the home topology to set their relative
  widths. The chosen ratio is remembered across reloads. Each side stops at a
  minimum width.
- **Drag the divider to open/close, not just resize.** Dragging the topology
  side fully closed returns to the chat; dragging the chat side fully closed
  makes the topology full-screen. The toolbar/sidebar buttons and the divider
  now drive the same layout state, so they always agree — closing the topology
  reliably returns the chat to full width.
- **Layout is in the URL.** The open layout is encoded as `?layout=chat`,
  `?layout=chat,topology`, or `?layout=topology`, so back/forward and shared
  links restore the exact arrangement (including full-screen topology). The
  topology panel keeps its live diagram (no reload/flicker) as the layout
  changes.
- Tightened the spacing and background treatment around the prompt input and the
