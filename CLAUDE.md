...
docs/                      # generated reference docs (see Areas Reference below)
...
| `update-areas-doc.py` | laptop | regenerate `docs/areas.md` from the live area/device/entity registries (see Areas Reference below) |
...
[`docs/areas.md`](docs/areas.md) lists every HA area's devices and entities,
grouped by domain, generated by [`bin/update-areas-doc.py`](bin/update-areas-doc.py).

**Not source of truth — a point-in-time snapshot.** Areas/devices/entities
live in HA's `.storage/` registries, not this repo, and can change any time
via the UI or automations. Use the doc to get oriented (what areas exist,
roughly what's in them) but **always verify against live HA** — `hass-cli`,
the MCP server, or `bin/update-areas-doc.py` — before writing YAML that
references an area, device, or entity ID. Don't take the doc's content as
current fact.
