...
//#region node_modules/.pnpm/@jetbrains+websandbox@1.2.1/node_modules/@jetbrains/websandbox/dist/websandbox.js
var t = /* @__PURE__ */ e(((e, t) => {
	(function(n, r) {
		...
		return (() => {
			...
			function i(e, n) {
				return t(n).reduce((e, t) => e ? e[t] : null, e);
			}
			let a = "message", o = "response", s = "set-interface", c = "service-message", l = !!window.MSInputMethodContext && !!document.documentMode, u = { allowedSenderOrigin: void 0 };
			class d {
				constructor(e, t, n = {}) {
					this.remote = {}, this.serviceMethods = {}, this.localApi = {}, this.callbacks = {}, this._resolveRemoteMethodsPromise = null, this.options = Object.assign(Object.assign({}, u), n), this.incrementalID = Math.floor(Math.random() * 1e5), this.postMessage = e, this.remoteMethodsWaitPromise = new Promise((e) => {
						this._resolveRemoteMethodsPromise = e;
					}), t((e) => this.onMessageListener(e));
				}
				onMessageListener(e) {
					let t = e.data, { allowedSenderOrigin: n } = this.options;
					n && e.origin !== n && !l || (t.type === o ? this.popCallback(t.callId, t.success, t.result) : t.type === a ? this.callLocalApi(t.methodName, t.arguments).then((e) => this.responseOtherSide(t.callId, e)).catch((e) => this.responseOtherSide(t.callId, e, !1)) : t.type === s ? (this.setInterface(t.apiMethods), this.responseOtherSide(t.callId)) : t.type === c && this.callLocalServiceMethod(t.methodName, t.arguments).then((e) => this.responseOtherSide(t.callId, e)).catch((e) => this.responseOtherSide(t.callId, e, !1)));
				}
				...
				setInterface(e) {
					...
					this.remote = {}, e.forEach((e) => {
						let n = t(e), r = this.remote;
						for (let e = 0; e < n.length - 1; e++) {
							let t = n[e];
							(!r[t] || typeof r[t] != "object") && (r[t] = {}), r = r[t];
						}
						r[n[n.length - 1]] = this.createMethodWrapper(e);
					}), (n = this._resolveRemoteMethodsPromise) == null || n.call(this);
				}
			...
			class g {
				...
				constructor(e, t) {
					this.connection = null, this.removeMessageListener = () => {}, this.validateOptions(t), this.options = Object.assign(Object.assign({}, h), t), this.iframe = this.createIframe(), this.promise = new Promise((t) => {
						this.connection = new f(this.iframe.contentWindow.postMessage.bind(this.iframe.contentWindow), (e) => {
							let t = (t) => {
				...
				_prepareFrameContent(e) {
					...
					return e.codeToRunBeforeInit && (t = t.replace("<head>", `<head><script>${e.codeToRunBeforeInit}<\/script>`) ?? ""), t = t.replace("<head>", `<head><script>${m()}<\/script>`) ?? "", e.initialStyles && (t = t.replace("</head>", `<style>${e.initialStyles}</style></head>`)), e.baseUrl && (t = t.replace("<head>", `<head><base target="_parent" href="${e.baseUrl}"/>`)), t;
				}
				createIframe() {
					...
					return n.sandbox = `allow-scripts ${this.options.sandboxAdditionalAttributes}`, n.allow = `${this.options.allowAdditionalAttributes}`, n.className = this.options.frameClassName ?? "", this.options.allowFullScreen && (n.allowFullscreen = !0), this.options.frameSrc ? (n.src = this.options.frameSrc, t.appendChild(n), n) : (n.setAttribute("srcdoc", this._prepareFrameContent(this.options)), t.appendChild(n), n);
				}
				destroy() {
