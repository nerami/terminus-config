...
import { n as t, r as n } from "./src-BYM73fzN.js";
import { T as r, b as i, x as a } from "./chunk-CSCIHK7Q-AEjI_JHf.js";
import { $ as o, J as s, K as c, Q as l, X as u, Y as d, Z as f, et as p, g as m, nt as h, q as g, rt as _, tt as v, u as y } from "./chunk-5ZQYHXKU--OC8flB4.js";
import { t as b } from "./line-CTjdCQim.js";
import { n as x } from "./chunk-O5CBEL6O-Br3PEsWJ.js";
import { i as S, n as ee, r as C, t as w } from "./chunk-BSJP7CBP-XfIAHpAE.js";
import { n as T } from "./chunk-L5ZTLDWV-DjGZgeb-.js";
import { i as E, n as D } from "./chunk-NZK2D7GU-C-653DhC.js";
import { t as te } from "./rough.esm-a8mysczE.js";
import { r as O } from "./chunk-3OPIFGDE-Dg_fnvX1.js";
...
var ne = /* @__PURE__ */ t((e, t, n, r, i, a = !1, o) => {
	...
	"dependency",
	"barb"
], j = /* @__PURE__ */ t((e, t, r, i, a, o, s = !1, c) => {
	...
	u.node().appendChild(f), n.info("abc82", i, i.labelType);
	let p = f.getBBox(), m = p;
	if (s) {
		let t = f.children[0], n = e(f);
		p = t.getBoundingClientRect(), m = p, n.attr("width", p.width), n.attr("height", p.height);
	} else {
		let t = e(f).select("text").node();
		t && typeof t.getBBox == "function" && (m = t.getBBox());
	}
	u.attr("transform", w(m, s)), M.set(i.id, l), i.width = p.width, i.height = p.height;
	let h;
	if (i.startLabelLeft) {
		let n = t.insert("g").attr("class", "edgeTerminals"), r = n.insert("g").attr("class", "inner"), a = await O(r, i.startLabelLeft, F(i.labelStyle) || "", !1, !1);
		h = a;
		let o = a.getBBox();
		if (s) {
		...
		r.attr("transform", w(o, s)), N.get(i.id) || N.set(i.id, {}), N.get(i.id).startLeft = n, L(h, i.startLabelLeft);
	}
	if (i.startLabelRight) {
		let n = t.insert("g").attr("class", "edgeTerminals"), r = n.insert("g").attr("class", "inner"), a = await O(r, i.startLabelRight, F(i.labelStyle) || "", !1, !1);
		h = a;
		let o = a.getBBox();
		if (s) {
			let t = a.children[0], n = e(a);
		...
		r.attr("transform", w(o, s)), N.get(i.id) || N.set(i.id, {}), N.get(i.id).startRight = n, L(h, i.startLabelRight);
	}
	if (i.endLabelLeft) {
		let n = t.insert("g").attr("class", "edgeTerminals"), r = n.insert("g").attr("class", "inner"), a = await O(n, i.endLabelLeft, F(i.labelStyle) || "", !1, !1);
		h = a;
		let o = a.getBBox();
		if (s) {
			let t = a.children[0], n = e(a);
		...
		r.attr("transform", w(o, s)), N.get(i.id) || N.set(i.id, {}), N.get(i.id).endLeft = n, L(h, i.endLabelLeft);
	}
	if (i.endLabelRight) {
		let n = t.insert("g").attr("class", "edgeTerminals"), r = n.insert("g").attr("class", "inner"), a = await O(n, i.endLabelRight, F(i.labelStyle) || "", !1, !1);
		h = a;
		let o = a.getBBox();
		if (s) {
			let t = a.children[0], n = e(a);
		...
		r.attr("transform", w(o, s)), N.get(i.id) || N.set(i.id, {}), N.get(i.id).endRight = n, L(h, i.endLabelRight);
	}
	return f;
}, "insertEdgeLabel");
function L(e, t) {
...
var R = /* @__PURE__ */ t((e, t) => {
	n.debug("Moving label abc88 ", e.id, e.label, M.get(e.id), t);
	let r = t.updatedPath ? t.updatedPath : t.originalPath, { subGraphTitleTotalMargin: i } = T(a());
	if (e.label) {
		let a = M.get(e.id), o = e.x, s = e.y;
		if (r) {
	...
	return i >= o || a >= s;
}, "outsideNode"), B = /* @__PURE__ */ t((e, t, r) => {
  ...
  node        : x:${e.x} y:${e.y} w:${e.width} h:${e.height}`);
	...
	if (Math.abs(a - t.y) * s > Math.abs(i - t.x) * l) {
		...
		return c === 0 && (i.x = t.x, i.y = t.y), d === 0 && (i.x = t.x), u === 0 && (i.y = t.y), n.debug(`abc89 top/bottom calc, Q ${u}, q ${e}, R ${d}, r ${c}`, i), i;
	} else {
...
}, "intersection"), V = /* @__PURE__ */ t((e, t) => {
	n.warn("abc88 cutPathAtIntersect", e, t);
	let r = [], i = e[0], a = !1;
	return e.forEach((e) => {
		if (n.info("abc88 checking point", e, t), !z(t, e) && !a) {
			let o = B(t, i, e);
			n.debug("abc88 inside", e, i, o), n.debug("abc88 intersection", o, t);
			let s = !1;
			r.forEach((e) => {
				s ||= e.x === o.x && e.y === o.y;
			}), r.some((e) => e.x === o.x && e.y === o.y) ? n.warn("abc88 no intersect", o, r) : r.push(o), a = !0;
		} else n.warn("abc88 outside", e, i), i = e, a || r.push(e);
...
var U = /* @__PURE__ */ t(function(e, t, n) {
...
}, "findAdjacentPoint"), ie = /* @__PURE__ */ t(function(e) {
	let { cornerPointPositions: t } = H(e), r = [];
	for (let i = 0; i < e.length; i++) if (t.includes(i)) {
		let t = e[i - 1], a = e[i + 1], o = e[i], s = U(t, o, 5), c = U(a, o, 5), l = c.x - s.x, u = c.y - s.y;
		r.push(s);
		let d = Math.sqrt(2) * 2, f = {
			x: o.x,
			y: o.y
		};
		Math.abs(a.x - t.x) > 10 && Math.abs(a.y - t.y) >= 10 ? (n.debug("Corner point fixing", Math.abs(a.x - t.x), Math.abs(a.y - t.y)), f = o.x === s.x ? {
			x: l < 0 ? s.x - 5 + d : s.x + 5 - d,
			y: u < 0 ? s.y - d : s.y + d
		} : {
			x: l < 0 ? s.x - d : s.x + d,
			y: u < 0 ? s.y - 5 + d : s.y + 5 - d
		}) : n.debug("Corner point skipping fixing", Math.abs(a.x - t.x), Math.abs(a.y - t.y)), r.push(f, c);
	} else r.push(e[i]);
	return r;
	...
	n.debug("UIO intersect check", r.points, M.x, j.x), M.intersect && j.intersect && !E && (k = k.slice(1, r.points.length - 1), k.unshift(j.intersect(k[0])), n.debug("Last point UIO", r.start, "-->", r.end, k[k.length - 1], M, M.intersect(k[k.length - 1])), k.push(M.intersect(k[k.length - 1])));
	let P = btoa(JSON.stringify(k));
	r.toCluster && (n.info("to cluster abc88", i.get(r.toCluster)), k = V(r.points, i.get(r.toCluster).node), A = !0), r.fromCluster && (n.debug("from cluster abc88", i.get(r.fromCluster), JSON.stringify(k, null, 2)), k = V(k.reverse(), i.get(r.fromCluster).node).reverse(), A = !0);
	...
	if (r.look === "handDrawn") {
		...
		H += " transition", U = e(i).select("path").attr("id", `${T}-${r.id}`).attr("class", " " + H + (r.classes ? " " + r.classes : "") + (X ? " " + X : "")).attr("style", q ? q.reduce((e, t) => e + ";" + t, "") : "");
		let a = U.attr("d");
	...
	let Q = "";
	(a().flowchart.arrowMarkerAbsolute || a().state.arrowMarkerAbsolute) && (Q = window.location.protocol + "//" + window.location.host + window.location.pathname + window.location.search, Q = Q.replace(/\(/g, "\\(").replace(/\)/g, "\\)")), n.info("arrowTypeStart", r.arrowTypeStart), n.info("arrowTypeEnd", r.arrowTypeEnd);
	let ae = !Z && r?.look === "neo";
	ne(U, r, Q, T, x, ae, Y);
