...
function NodeDetailContent({ node }: { node: GraphNode }) {
  ...
  const lookupId =
    node.kind === "entity" || node.kind === "template" || node.kind === "scene" || node.kind === "script"
      ? node.id
      : undefined
  ...
  return (
    <>
      <PopoverHeader>
        <PopoverTitle className="font-mono text-sm">{node.id}</PopoverTitle>
...
export function NodeDetailSheet({ open, onOpenChange, node, anchor }: Props) {
  if (!node) return null

  return (
    <Popover open={open} onOpenChange={onOpenChange}>
      <PopoverContent
        className="w-[360px] overflow-y-auto max-h-[80vh]"
